...
export default function Edit() {
    ...
    const loans = useFetch(endPoints.loans.loans_list);
    return (
        <>
            <NavBar />
                                ...
                                return (
                                    <>
                                        <div class="grid grid-cols-5 gap-4">
                                            <div class="grid grid-rows-2">Ultimo dia de pago</div>
                                            <div class="grid grid-rows-2">Fecha Pago Mensual</div>
                                            <div class="grid grid-rows-2">Tipo Prestamo</div>
                                            <div class="grid grid-rows-2">Penalidad</div>
                                            <div class="grid grid-rows-2">Total Pago</div>

                                            <div class="grid grid-rows-2">{item.lastDayPayment}</div>
                                            <div class="grid grid-rows-2">{item.datePaymentMonthly}</div>
                                            <div class="grid grid-rows-2">{item.typeLoan}</div>
                                            <div class="grid grid-rows-2">{item.penalty}</div>
                                            <div class="grid grid-rows-2">{item.totalPayment}</div>
                                        </div>
                                        
                                        <p class="italic ...">Total Prestamo {item.amountPrice}</p>
                                        <div class="bg-gray-200 rounded-full dark:bg-gray-700 w-full">
                                            <div class="bg-blue-600 text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full w-32" style={{ width: `${item.amountPrice}%` }}>{item.amountPrice}</div>
                                        </div>
                                    </>
                                )
                        })}
