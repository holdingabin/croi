...
export default function Edit() {
    ...
    const investment = useFetch(endPoints.investment.investment_list);
    return (
        <>
            <NavBar />
                                ...
                                return (
                                    <>
                                        <div class="grid grid-cols-2 gap-4">
                                            <div class="grid grid-rows-2">Dia del bono</div>
                                            <div class="grid grid-rows-2">{item.date}</div>
                                        </div>
                                        
                                        <p class="italic ...">Total Actions {item.totalAmount}</p>
                                        <div class="bg-gray-200 rounded-full dark:bg-gray-700 w-full">
                                            <div class="bg-blue-600 text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full w-32" style={{ width: `${item.remainingAmount}%` }}>{item.remainingAmount}</div>
                                        </div>
                                        <div className="pt-12 pb-8">
                                            <button className="bg-green-700 hover:bg-green-900 text-white font-bold py-2 px-4 rounded-full">
                                                <Link href={`invetmentbuy/[id]`} as={`invetmentbuy/${item.id}`}>
                                                    Inverciones echas
                                                </Link>
                                            </button>
                                        </div>
