...
export default function Edit() {
    ...
    const actions = useFetch(endPoints.actions.actions_list);
                                ...
                                return (
                                    <>
                                        <div class="grid grid-cols-2 gap-4">
                                            <div class="grid grid-rows-2">Precio de Compra</div>
                                            <div class="grid grid-rows-2">Precio de Venta</div>
                                            <div class="grid grid-rows-2">{item.purchasePrice}</div>
                                            <div class="grid grid-rows-2">{item.sellingPrice}</div>
                                        </div>

                                        <p class="italic ...">Total Actions {item.nActionsSales}</p>
                                        <div class="bg-gray-200 rounded-full dark:bg-gray-700" style={{ width: `${item.nActionsSales}%` }}>
                                            <div class="bg-blue-600 text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full w-32" style={{ width: `${item.nActionsRest}%` }}>{item.nActionsRest}</div>
                                        </div>

                                        <div className="pt-12 pb-8">
                                            <button className="bg-green-700 hover:bg-green-900 text-white font-bold py-2 px-4 rounded-full">
                                                <Link href={`actionbuy/[id]`} as={`actionbuy/${item.id}`}>
                                                    Acciones vendidas
                                                </Link>
                                            </button>
                                        </div>
